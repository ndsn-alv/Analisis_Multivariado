...
# 6. Análisis de correspondencias múltiple (MCA)

```{r}
## Se elimina: referida, motivo_ile, alcaldia / municipio, recibio_consejeria

ile_cat <- ile %>%
  dplyr::select(estado_civil, referida, 
                institucion_derechohab, nivel_edu,
                ocupacion, religion, entidad,
                recibio_consejeria, se_complica,firma_consentimiento,
                con_dolor, analgesico, procedimiento_ile, edad_inicio_vida_sexual, edad_primera_menstruacion, edad, numero_embarazos, numero_hijos, numero_abortos, numero_iles, numero_partos, numero_cesareas) %>%
  mutate(
    ...
    institucion_derechohab = case_when(
      institucion_derechohab %in% c("IMSS", "ISSSTE", "Otra", "Seguro Popular") ~ "Alguno",
      institucion_derechohab == "Ninguno" ~ "Ninguno",
      TRUE ~ NA
    ),
    ...
    nivel_edu = case_when(
      ...
      nivel_edu %in% c("Primaria", "Secundaria") ~ "Secundaria o inferior",
      TRUE ~ NA
    ),

    religion = case_when(
      religion == "Católica" ~ "Católica",
    ...
    ocupacion = case_when(
      ocupacion %in% c("Desempleada", "Empleada", "Estudiante", "Trabajadora del hogar no remunerada") ~ ocupacion,
      TRUE ~ NA
    ),

    analgesico_dolor = case_when(
      con_dolor == "Si" & analgesico == "Si" ~ "SS",
      con_dolor == "Si" & analgesico == "No" ~ "SN",
      con_dolor == "No" & analgesico == "Si" ~ "NS",
      con_dolor == "No" & analgesico == "No" ~ "NN",
      TRUE ~ NA
    ),
    ...
    edad_inicio_vida_sexual = case_when(
      is.na(edad_inicio_vida_sexual) ~ NA_character_,
      edad_inicio_vida_sexual <= 11 ~ "11-",
      edad_inicio_vida_sexual <= 14 ~ "12-14",
      edad_inicio_vida_sexual <= 16 ~ "15-16",
      edad_inicio_vida_sexual <= 18 ~ "17-18",
      edad_inicio_vida_sexual >= 19 ~ "19+"
    ),

    edad_primera_menstruacion = case_when(
      is.na(edad_primera_menstruacion) ~ NA_character_,
      edad_primera_menstruacion <= 8 ~ "8-",
      edad_primera_menstruacion <= 10 ~ "9-10",
      edad_primera_menstruacion <= 12 ~ "11-12",
      edad_primera_menstruacion <= 14 ~ "13-14",
      edad_primera_menstruacion >= 15 ~ "15+"
    )
  ) %>% mutate(
  numero_embarazos = case_when(
    numero_embarazos >= 5 ~ "5+",
    TRUE ~ as.character(numero_embarazos)
  ),
  numero_iles = case_when(
    numero_iles >= 1 ~ "1+",
    TRUE ~ as.character(numero_iles)
  ),
  numero_abortos = case_when(
    numero_abortos >= 2 ~ "2+",
    TRUE ~ as.character(numero_abortos)
  ),
  numero_hijos = case_when(
    numero_hijos >= 3 ~ "3+",
    TRUE ~ as.character(numero_hijos)
  ),
  numero_partos = case_when(
    numero_partos >= 3 ~ "3+",
    TRUE ~ as.character(numero_partos)
  ),
  numero_cesareas = case_when(
    numero_cesareas >= 2 ~ "2+",
    TRUE ~ as.character(numero_cesareas)
  )
) %>% dplyr::select(
  -analgesico, -con_dolor, -referida, -firma_consentimiento, -recibio_consejeria, 
  -entidad, -edad_inicio_vida_sexual, -edad_primera_menstruacion, - numero_embarazos, -numero_hijos)

ile_mca <- MCA(ile_cat, graph = FALSE)

color_var <- ile_cat$analgesico_dolor
...
wrap_plots(mcav1, mcabp1, ncol = 2)
wrap_plots(mcav2, mcabp2, ncol = 2)
wrap_plots(mcav3, mcabp3, ncol = 2)
```
